...
df %>%
  ggplot(aes(x=reorder(groupe,nombre),y=nombre,fill=Série.Domaine.Filière)) +
    ...
    guides(fill=FALSE) +
    ggtitle("Catégorisation des candidatures avec l'ancien Bac général (séries + spécialités)") +
    theme_cpesr_cap() +
    theme(
      panel.grid.major.y = element_blank(),
      panel.grid.major.x = element_line(color="grey",size=0.2),
...
all %>%
  mutate(categorisation = factor(categorisation, levels=rev(c("Série","Série + Spécialité","Spécialités de Terminale", "Spécialités de Terminale + options","Spécialités de 1ère et Term. + options")))) %>% 
