...
palette_sum <- rep(brewer.pal(2, "Paired")[1:2],nrow(all)/2+1)[1:nrow(all)]
...
all %>%
  ...
  mutate(cat = reorder(paste(categorisation,categorie),nombre)) %>%
  ggplot(aes(x=categorisation,y=nombre,fill=cat)) +
    geom_col(position="fill") +
    coord_flip() +
    scale_fill_manual(values=rev(palette_sum)) +
